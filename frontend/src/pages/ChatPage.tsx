      ...
      <div className="max-w-4xl w-full h-[600px] flex gap-4">
        ...
        <div className="w-64 flex flex-col rounded-xl flex-shrink-0 overflow-hidden" style={{ backgroundColor: "var(--color-bg-card)", border: "1px solid var(--color-border)" }}>
          ...
          <div className="px-4 pt-3 pb-1 flex items-center justify-between">
            <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--color-text-muted)" }}>Direct Messages</span>
          </div>
          <div className="flex-1 overflow-y-auto px-2 pb-1 space-y-0.5 min-h-0">
            {dmChannels.length === 0 && (
              <p className="text-[11px] px-2 py-2" style={{ color: "var(--color-text-muted)" }}>No DMs yet — find someone to message!</p>
            )}
            {dmChannels.map((ch) => {
            ...
            {publicChannels.length > 0 && (
              <>
                <div className="flex items-center justify-between pt-3 pb-1">
                  <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--color-text-muted)" }}>Channels</span>
                  <button
                ...
                {showCreate && (
                  <div className="p-2 space-y-1.5 mb-1 rounded-lg" style={{ backgroundColor: "var(--color-bg-input)" }}>
                    ...
                    <button onClick={handleCreateChannel} disabled={!newName.trim()}
                      className="w-full py-1 rounded text-xs font-medium text-white disabled:opacity-40"
        ...
        <div className="flex-1 relative">
              ...
              <div className="rounded-xl p-4 mx-4 shadow-2xl" style={{ backgroundColor: "var(--color-bg-card)" }}>
                ...
                <div className="flex gap-2 justify-end">
                  <button onClick={() => setKickConfirm(null)}
                    className="px-3 py-1.5 rounded text-xs" style={{ backgroundColor: "var(--color-bg-input)", color: "var(--color-text-primary)" }}>Cancel</button>
                  <button onClick={handleKickConfirm}
                    className="px-3 py-1.5 rounded text-xs text-white" style={{ backgroundColor: "#DC2626" }}>Kick</button>
                </div>
              </div>
            </div>
          )}
          ...
          {activeView === "friends" ? (
            ...
            activeChannelData?.channel_type === "dm" ? (
                  ...
                  {members.map((m) => (
                    <div key={m.id} className="flex items-center justify-between px-2 py-1.5 rounded text-xs" style={{ backgroundColor: "var(--color-bg-input)" }}>
                      <div className="flex items-center gap-2 min-w-0 cursor-pointer" onClick={() => navigate(`/profile/${m.user}`)}>
                        ...
                        {m.role === "owner" && (
                          <><span className="text-[9px] px-1 rounded" style={{ backgroundColor: "#FCD34D20", color: "#FCD34D" }}>OWNER</span><span className="text-[9px] px-1 rounded" style={{ backgroundColor: "#60A5FA20", color: "#60A5FA" }}>ADMIN</span></>
                        )}
                ...
                {isOwner ? (
                  <div className="px-3 py-2 border-t" style={{ borderColor: "var(--color-border)" }}>
                    <button onClick={handleDeleteChannel}
                      className="w-full py-1.5 rounded text-xs font-medium text-white" style={{ backgroundColor: "#DC2626" }}>Delete Channel</button>
                  </div>
                  ...
                  <div className="px-3 py-2 border-t" style={{ borderColor: "var(--color-border)" }}>
                    <button onClick={async () => { if (!activeChannel) return; try { await leaveChannel(activeChannel); setChannels((prev) => prev.filter((c) => c.id !== activeChannel)); setActiveChannel(null); setShowSettings(false); } catch {} }}
                      className="w-full py-1.5 rounded text-xs font-medium" style={{ backgroundColor: "#F59E0B", color: "white" }}>Leave Channel</button>
                  </div>
                ) : null}
              </div>
            )
            ...
            activeChannel ? (
              <div onClick={handleMarkRead} className="h-full">
                <ChatWindow
                  ...
                  onSettingsClick={() => { fetchMembers(activeChannel).then(setMembers).catch(() => {}); setShowSettings(true); }}
